    ...
    return (
        <form >
            ...
            <div className="mb-3">
                ...
                <input onChange={e => setItem(item ? {...item, category : e.target.value} : {id : 0, name : '', category : e.target.value, price : 0})} value={item?.category || ''} type="text" className="form-control" placeholder="Enter Category" />
            </div>

            <div className="mb-3">
                <label className="form-label">Price</label>
                <input onChange={e => setItem(item ? {...item, price : Number.parseInt(e.target.value)} : {id : 0, name : '', category : '', price : Number.parseInt(e.target.value)})} value={item?.price || 0} type="number" className="form-control" placeholder="Enter Price" />
            </div>
...
function ItemList ({list, onAction} : {list : Item[], onAction?:ItemAction}) {
        ...
        if(list.length === 0){
            return <span>There is no list</span>
        }
